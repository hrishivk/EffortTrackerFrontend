...
const TeamOverview: React.FC = () => {
  const teamData = [
    {
      ...
      AssignedProjects: "Project Alpha",
      start: 3,
      inprogress: 5,
      complete: 8,
      total: 16,
    ...
    },
    {
      user: "Emily Davis",
      avatar: "/placeholder.svg?height=32&width=32",
      AssignedProjects: "Project Delta",
      start: 4,
      inprogress: 2,
      complete: 9,
      total: 15,
  ...
  const UserAvatar = ({ user, avatar }: { user: string; avatar: string }) => {
    const initials = user
      .split(" ")
      .map((n) => n[0])
      .join("")
    ...
    <div className="w-full max-w-7xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track team progress and task distribution</p>
        </div>
        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 border border-gray-300">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50/50">
                <th className="font-semibold text-gray-700 py-4 px-6 text-left">Team Member</th>
                <th className="font-semibold text-gray-700 py-4 px-4 text-left">Project</th>
                ...
                <th className="font-semibold text-gray-700 py-4 px-4 text-center">Progress</th>
                <th className="font-semibold text-gray-700 py-4 px-4 text-center">Status</th>
              </tr>
            </thead>
            <tbody>
              {teamData.map((member, index) => (
                <tr key={index} className="hover:bg-gray-50/50 transition-colors border-b border-gray-100">
                  <td className="py-4 px-6">
                    <UserAvatar user={member.user} avatar={member.avatar} />
                  </td>
